...
如果存在多个多倍体物种，导致单拷贝基因家族的数量为0，无法运行当前流程。解决方案如下

- 异源多倍体，拆分亚基因组，例如小麦，可以拆分成 tae_a.pep, tae_b, tae_d 三套，然后重新运行OrthoFinder。
- 同源多倍体，选择其中一套基因组，例如栽培种马铃薯、紫花苜蓿、甘蔗

> 要求高质量的基因组, 必须是染色体级别。草图级别的基因组建议不纳入分析。
### 如何选择校准点
...
ASTRAL 基于四分体方法，四分体本身是无根的, 软件只是为了方便显示，会所以选择一个物种作为根, 这个根不代表真实的演化起点.

**注意**: ASTRAL不适用中点定根方法，这是因为中点定根的假设是演化速率恒定，即从根到各叶节点的分子演化距离大致相等，但 ASTRAL 的枝长反映的是基因树与物种树的不一致程度，不是分子演化距离，所以这个假设完全不成立。

IQ-TREE/RAxML的枝长单位是替换数/位点，是可以用中点定根法，但更加建议选择一个外群。
